...
opencan-daemon (单进程, Go binary)
    ...
    │
    ├── SessionManager
    │     ├── session_id_1 → ACPProxy { process, state, event_buffer, client(ownerID) }
...
→ { "jsonrpc": "2.0", "id": 3, "method": "daemon/session.attach",
    "params": { "sessionId": "sess-abc123", "lastEventSeq": 0, "clientId": "ios-install-uuid" } }
...
`daemon/session.attach` 的所有权语义：
- 同一 session 同时只允许一个 attached 连接。
- 如果新连接携带的 `clientId` 与当前 owner 一致，daemon 允许“同 owner 重连接管”（把旧连接原子替换为新连接）。
- 不同 `clientId` 的并发 attach 会被拒绝，保持严格单 owner。
...
type ACPProxy struct {
    ...
    cmd         *exec.Cmd                 // ACP 子进程
    stdin       io.WriteCloser            // 写入 ACP 进程
    scanner     *bufio.Scanner            // 读取 ACP stdout

    eventBuffer *EventBuffer              // 缓存 session/update
    client      atomic.Pointer[clientWrapper] // attached client + ownerID

    pendingRequests map[int64]PendingRequest // request ID 重写映射
    doneCh          chan struct{}            // ACP 退出信号
}

type clientWrapper struct {
    conn    ClientConn
    ownerID string // 对应 daemon/session.attach 的 clientId
}
```
...
#### Attach 所有权转移 (clientId / ownerID)

`daemon/session.attach` 现在支持“同 owner 重连接管”：

- iOS 在安装后生成一次稳定 `clientId`（`daemonAttachClientID`），持久化在 `UserDefaults`。
- 每次 attach 都携带该 `clientId`，daemon 保存为 `ownerID`。
- `ACPProxy.TryAttachClientWithOwner` 使用 CAS（`atomic.Pointer`）处理并发 attach/detach。
...
for {
    current := client.Load()
    desired := &clientWrapper{conn: newConn, ownerID: ownerID}
    switch {
    case current == nil:
        if CAS(nil, desired) { return attached }
    case current.conn == newConn:
        if CAS(current, desired) { return attached } // 刷新 owner 元数据
    case ownerID != "" && current.ownerID == ownerID:
        if CAS(current, desired) { return attached } // 同 owner 重连接管
    default:
        return rejected // 不同 owner，拒绝并发 attach
    }
}
```

为什么必须 CAS：旧连接断开时 `DetachClient` 与新连接 `attach` 可能并发发生。CAS 可确保“读到的 current 仍是 current”时才写入，避免无锁 `Store` 覆盖导致 owner 丢失或错误抢占。
...
5. 一段时间后，iOS 重连:
   ...
   → iOS 发 daemon/session.attach { sessionId, lastEventSeq: 42, clientId }
   → 若旧连接尚未清理，但 ownerID 与 clientId 一致，daemon 原子转移所有权到新连接
...
5. 切换 workspace:
   ...
   → daemon/session.attach { sessionId: "session-2", clientId }
   → 无需新的 SSH 连接
```
...
3. **事件时序与重放**
   ...
   - `daemon/session.attach(lastEventSeq[, clientId])` 必须返回所有 `seq > lastEventSeq` 的缓存事件，且顺序保持与原始到达一致。
...
func TestDaemon_DisconnectAndReattach(t *testing.T) {
    ...
    sendJSON(conn2, `{"method":"daemon/session.attach","params":{"sessionId":"...","lastEventSeq":3,"clientId":"ios-install-uuid"}}`)
    resp := readJSON(conn2)
